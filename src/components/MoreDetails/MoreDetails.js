...
function MoreDetails({ closeMoreDetails }) {
  ...
  return (
    <div className="more-details">
      ...
      <p className="poke-name">{detailsData.name}</p>
      <div className="details">
        <div className="types">
          <h3>Types</h3>
          <div className="item-types">{detailsData.types ? detailsData.types.map((item) => <p key={item.slot}>{item.type.name}</p>) : null}</div>
        </div>
        <div className="abilities">
          <h3>Abilities</h3>
          <div className="item-abilities">{detailsData.abilities ? detailsData.abilities.map((item, index) => <p key={index}>{item.ability.name}</p>) : null}</div>
        </div>
        <div className="stats">
          <h3>Stats</h3>
